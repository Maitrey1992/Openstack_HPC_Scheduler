...
class HpcScheduler(driver.Scheduler):
    ...
    def schedule_run_instance(self, context, request_spec,
        ...
        self.notifier.info(context, 'hpc.scheduler.run_instance.start', payload)
        ...
        LOG.info(_("From Controller node HPC Request Spec: %s") % request_spec)
...
#	"""  store previously selected host id and use it for future allocation of instances """
            if num > 0:
	    	if prev_host.to_dict()['host'] in str(weighed_hosts) :
	    		chosen_host = prev_host
			selected_hosts.append(chosen_host)
			LOG.info("HPC Scheduler second attempt")
		else :
			pt = type(prev_host)
			wh = type(weighed_hosts)
			#temp = 
#			LOG.info(_("prev host type = %s weighed host type = %s") %(pt,wh))
			LOG.info(_("Previous Host %s and weighed host %s") %(prev_host.to_dict()['host'],weighed_hosts))
			LOG.info("HPC Scheduler : No host found after first request")
			break
	    if num == 0 :
		chosen_host = random.choice(weighed_hosts[0:scheduler_host_subset_size])
		selected_hosts.append(chosen_host)
	    	prev_host = chosen_host
		#prev_host = prev_host.to_dict()
	    	LOG.info(_("First Iteration HPC Scheduler %s") % chosen_host)
		LOG.info(_("Previous host type %s")%type(prev_host))
